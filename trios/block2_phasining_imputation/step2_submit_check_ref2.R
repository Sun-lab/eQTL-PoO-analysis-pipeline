...
for(i in c(1:22)){
  ...
  file.copy(from = sprintf("%s/chri/geno_chr%s.map",geno.dir,i), to = "tmp.map")
  file.copy(from = sprintf("%s/chri/geno_chr%s.ped",geno.dir,i), to = "tmp.ped")
  system("plink --file tmp --flip tmp_strand.txt  --recode")
  cat("plink done ")
  
  con = file(sprintf("tmp.ped",geno.dir,i))
  tmp = readLines(con)
  close(con)
  cat("\t read preconv ")

  con = file(sprintf("plink.ped",geno.dir,i))
  tmp0 = readLines(con)
  close(con)
  cat("\t read postconv: ")
