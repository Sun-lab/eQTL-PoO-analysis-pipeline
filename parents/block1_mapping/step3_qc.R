...
maxb = 76
  ...
  fls = list.files(data.dir, "fastq$", full=TRUE)
  ...
  dest.dir = sprintf("out/fast_qc_report")
  ...
  mean = median = upper = lower = matrix(nrow=maxb, ncol=1)
  ...
  pdf(sprintf("%s/perCycleScore_mean_%s.pdf",dest.dir,subi), width=8, height=4)
  ...
  pdf(sprintf("%s/perCycleScore_median_%s.pdf",dest.dir,subi), width=8, height=4)
  par(mar=c(5,4,2,1), bty="n")
  ...
  pdf(sprintf("%s/perCycleScore_25_%s.pdf",dest.dir,subi), width=8, height=4)
  par(mar=c(5,4,2,1), bty="n")
